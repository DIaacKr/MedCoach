...
@click.option(
    ...
    "-c",
    type=str,
    default="exps/configs/base.yaml",
)
@click.option(
...
def main(
    ...
    input_data = load_huatuo_eval_data(config.eval_data_path, limit=config.limit)
    ...
    if config.prefix_prompt is not None and config.prefix_prompt:
...
def load_huatuo_eval_data(file_path, limit=0):
    ...
    if limit > 0:
        print(f"Limit each dataset to {limit} samples.")

    for k, v in data.items():
        if limit > 0 and len(v) > limit:
            print(f"Randomly sampling dataset '{k}' from {len(v)} to {limit} samples.")
            v = random.sample(v, limit)
        for da in v:
            da["source"] = k
        input_data.extend(v)
    return input_data
